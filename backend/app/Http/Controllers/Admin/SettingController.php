    ...
    public function update(Request $request): RedirectResponse
    {
        ...
        try {
            $data = $request->validate([
                ...
                'home_logo_url' => ['nullable', 'url', 'max:2048'],
        ...
        $currentHomeLogo = (string) Setting::getValue(Constants::SETTING_HOME_LOGO, '');
        $homeLogoUploaded = $request->file('home_logo');
        $homeLogoUrl = trim((string) ($data['home_logo_url'] ?? ''));
        if ($homeLogoUploaded && $homeLogoUploaded->isValid()) {
            $oldPath = ! str_starts_with($currentHomeLogo, 'http') && ! str_starts_with($currentHomeLogo, 'data:')
                ? $currentHomeLogo
                : null;
            $path = Media::store($homeLogoUploaded, 'settings', $oldPath);
            if (! $path) {
                throw ValidationException::withMessages([
                    'home_logo' => 'تعذّر حفظ شعار الرئيسية.',
                ])->redirectTo(route('admin.settings.index', ['tab' => $tab]));
            }

            Setting::setValue(Constants::SETTING_HOME_LOGO, $path);
        } elseif ($homeLogoUrl !== '') {
            if ($currentHomeLogo && $homeLogoUrl !== $currentHomeLogo) {
                Media::delete($currentHomeLogo);
            }
