...
class PushService with WidgetsBindingObserver {
  ...
  Future<void> initialize({required NotificationsCubit cubit}) async {
    ...
    if (messaging == null) {
      debugPrint('[Push] Firebase is not initialized — skip push setup.');
      return;
    }
  ...
  Future<void> sync() async {
    ...
    if (AuthSession.instance.user?.notificationsEnabled == false) {
      _poll?.cancel();
      return;
    }
    _startPoll();
  ...
  Future<void> setEnabled(bool enabled) async {
    ...
    await applyPreference(enabled);
  }

  /// يُستدعى بعد حفظ التفضيل على الخادم — لا يعيد طلب الـ API.
  ...
  Future<void> _registerIfAllowed(String token) async {
    ...
    try {
      ...
      debugPrint('[Push] register failed: $detail');
    }
  }
