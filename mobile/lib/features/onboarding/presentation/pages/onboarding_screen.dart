...
List<_OnboardingData> _pagesFromApi(List<OnboardingSlideData> slides) {
  const accents = [
    Color(0xFF4CAF50),
    Color(0xFF2196F3),
    Color(0xFF9C27B0),
  ...
  const gradients = [
    ...
    [Color(0xFFE65100), Color(0xFFF57C00), Color(0xFFFFB74D)],
    [Color(0xFF004D40), Color(0xFF00796B), Color(0xFF4DB6AC)],
  ];
  const icons = [
...
class _OnboardingScreenState extends State<OnboardingScreen>
  ...
  void initState() {
    super.initState();
    final remote = StartupApi.instance.cached?.onboarding ?? const [];
    _pages = remote.isNotEmpty ? _pagesFromApi(remote) : _fallbackPages;
    ...
    // إن لم تُحمَّل من السبلاش، حاول مرة أخرى بهدوء
    if (remote.isEmpty) {
      StartupApi.instance.fetch().then((payload) {
        if (!mounted || payload.onboarding.isEmpty) return;
        setState(() {
          _pages = _pagesFromApi(payload.onboarding);
          _currentPage = 0;
        });
      });
    }
        ...
        children: [
          ...
          SafeArea(
            child: Align(
              ...
              child: AnimatedOpacity(
                ...
                child: TextButton(
                  onPressed:
                      _currentPage < pages.length - 1 ? _launchApp : null,
                  child: const Text(
                    'تخطي',
                    style: TextStyle(
          ...
          Positioned(
            ...
            child: _BottomBar(
              ...
              accentColor: pages[_currentPage].accentColor,
              onNext: _nextPage,
      ...
      child: SafeArea(
        child: Column(
          children: [
            ...
            // ── صورة توضيحية (صورة من الأدمن أو أيقونة) ──────────────────────
            AnimatedBuilder(
                      ...
                      ? ClipRRect(
                          ...
                          child: SizedBox(
                            ...
                            child: AppNetworkImage(
                              data.imageUrl,
                              fit: BoxFit.cover,
                              error: _IllustrationWidget(data: data),
                            ),
                          ),
                        )
                      : _IllustrationWidget(data: data),
                ),
              ),
            ),
