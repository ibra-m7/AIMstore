...
class DeliveryApi {
  ...
  Future<DeliverySlotsCalendar> slots() async {
    final json = await _client.get('/delivery/slots', auth: true);
    final data = json['data'];
    if (data is! Map) {
      throw ApiException(
        (json['message'] as String?) ?? 'تعذّر تحميل فترات التوصيل.',
      );
    }
    return DeliverySlotsCalendar.fromJson(Map<String, dynamic>.from(data));
  }
}
