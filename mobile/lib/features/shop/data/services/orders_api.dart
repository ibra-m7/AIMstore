...
class OrdersApi {
  ...
  Future<OrderEntity> create({
    ...
    final codes = <String>[
      ...?couponCodes?.map((c) => c.trim().toUpperCase()).where((c) => c.isNotEmpty),
      if (couponCode != null && couponCode.trim().isNotEmpty)
        couponCode.trim().toUpperCase(),
    ];
    final uniqueCodes = <String>{...codes}.toList();
      ...
      '/orders',
      {
        ...
        if (bundles.isNotEmpty)
          'bundles': bundles
              .map(
                (line) => {
                  'bundle_id': int.tryParse(line.bundle.id) ?? line.bundle.id,
        ...
        if (uniqueCodes.isNotEmpty) 'coupon_codes': uniqueCodes,
        if (uniqueCodes.isNotEmpty) 'coupon_code': uniqueCodes.first,
