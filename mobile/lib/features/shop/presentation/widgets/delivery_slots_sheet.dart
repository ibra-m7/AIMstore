...
class DeliverySlotsSheet {
  static Future<DeliverySlotSelection?> show(
    BuildContext context, {
    DeliverySlotSelection? initial,
  }) {
    return showCheckoutSheet<DeliverySlotSelection>(
      context: context,
      builder: (_) => _DeliverySlotsBody(initial: initial),
    );
  }
}

class _DeliverySlotsBody extends StatefulWidget {
  final DeliverySlotSelection? initial;
  const _DeliverySlotsBody({this.initial});

  @override
  State<_DeliverySlotsBody> createState() => _DeliverySlotsBodyState();
}

class _DeliverySlotsBodyState extends State<_DeliverySlotsBody> {
  static const double _itemExtent = 48;
  static const double _wheelHeight = 220;

  DeliverySlotsCalendar? _calendar;
  String? _error;
  bool _loading = true;
  int _dayIndex = 0;
  int _slotIndex = 0;
  ...
  List<DeliveryDay> get _days => _calendar?.days ?? const [];

  List<DeliverySlot> _slotsForDay(int dayIndex) {
    final days = _days;
  ...
  Future<void> _load() async {
    ...
    try {
      final calendar = await DeliveryApi.instance.slots();
      if (!mounted) return;
      ...
      if (widget.initial?.day != null) {
        final i = days.indexWhere(
          (d) => d.date == widget.initial!.day!.date,
        );
        if (i >= 0) dayIndex = i;
      }
      ...
      setState(() {
        _loading = false;
        _error = 'تعذّر تحميل فترات التوصيل.';
      });
    }
  }
  ...
  void _continue() {
    ...
    if (slots.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('اختر فترة توصيل متاحة', textAlign: TextAlign.center),
        ),
      );
      return;
    }
    ...
    Navigator.of(context).pop(
      DeliverySlotSelection.scheduled(day: day, slot: slot),
    );
  }
                  ...
                  const Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        SizedBox(height: 4),
                        Text(
                          'مرّر لاختيار اليوم والساعة المناسبة',
                          style: TextStyle(
                            fontSize: 12.5,
